...
def hero_html(lang: str) -> str:
    ...
    badge = "اے آئی خواندگی · پوسٹ ۱۴" if is_ur else "AI Literacy · Post 14"
    hook = (
        "اے آئی مسودہ ہے یا فیصلہ؟"
        if is_ur
        else 'Is AI the draft, or the <span class="hl">decision</span>?'
    )
    sub = "جانچو، پھر بھیجو" if is_ur else "Verify, then send"
    panel = "بھیجا · جانچا" if is_ur else "Sent · verified"
    b_tag = "بھیجا؟" if is_ur else "Sent?"
    b_h = "پڑھے بغیر بھیجا" if is_ur else "Sent unread"
    b_d = "مسودہ، بغیر جانچ" if is_ur else "Draft, unchecked"
    b_mega = "غلط عدد گیا" if is_ur else "Wrong number went out"
    b_status = "خطرہ قبول" if is_ur else "Risk accepted"
    r_tag = "جانچا؟" if is_ur else "Verified?"
    r_h = "جانچ کر بھیجا" if is_ur else "Verified, then sent"
    r_d = "حقیقت · عدد · نام" if is_ur else "Fact · number · name"
    checks = (
        [("۱", "حقیقت"), ("۲", "عدد"), ("۳", "نام")]
        if is_ur
        else [("1", "Fact"), ("2", "Number"), ("3", "Name")]
    )
    r_status = "آپ کا فیصلہ" if is_ur else "Your decision"
    take = (
        "مسودہ اے آئی سے، فیصلہ اور ذمہ داری آپ کی"
        if is_ur
        else "The draft can come from AI. The decision and responsibility stay with you."
    )
    footer = (
        "پاکستان میڈیا او ایس · اے آئی خواندگی · پوسٹ ۱۴"
        if is_ur
        else "Pakistan Media OS · AI Literacy · Post 14"
    )
    check_rows = "".join(
...
def shell(lang: str, dark: bool, title: str, body_html: str, footer: str) -> str:
    ...
    series = "اے آئی خواندگی · پوسٹ ۱۴" if lang == "ur" else "AI Literacy · Post 14"
...
FOOT_UR = "پاکستان میڈیا او ایس · اے آئی خواندگی · پوسٹ ۱۴"
FOOT_EN = "Pakistan Media OS · AI Literacy · Post 14"

SLIDES_UR = [
    {
        "name": "POST-014-slide-ur-01.png",
        "dark": True,
        "title": "اے آئی مسودہ ہے یا فیصلہ؟",
        "body": '<p class="sub">جانچو، پھر بھیجو</p><p class="sub" style="margin-top:18px">پوسٹ ۱۴</p>',
    },
    {
        "name": "POST-014-slide-ur-02.png",
        "dark": False,
        "title": "پڑھے بغیر بھیج دیا",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>اے آئی کی رپورٹ پیسٹ کی</span></li>'
            '<li><span class="dot"></span><span>غلط عدد شامل تھا</span></li>'
            '<li><span class="dot"></span><span>کلائنٹ نے پکڑ لیا</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-ur-03.png",
        "dark": False,
        "title": "پوسٹ ۱۳ سے",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>جہاں مثالیں کم، وہاں پراعتماد غلط جواب</span></li>'
            '<li><span class="dot"></span><span>آج: جانچنے کی عادت</span></li>'
            '<li><span class="dot"></span><span>عادت جاری رہتی ہے</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-ur-04.png",
        "dark": False,
        "title": "اصل تقسیم",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>اے آئی مسودہ دے</span></li>'
            '<li><span class="dot"></span><span>آپ جانچیں</span></li>'
            '<li><span class="dot"></span><span>آپ فیصلہ کریں، ذمہ داری آپ کی</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-ur-05.png",
        "dark": False,
        "title": "عام غلطیاں ۱ تا ۳",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>کاپی پیسٹ جمع کروانا</span></li>'
            '<li><span class="dot"></span><span>غیر تصدیق شدہ حقائق</span></li>'
            '<li><span class="dot"></span><span>بڑے فیصلے آؤٹ سورس کرنا</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-ur-06.png",
        "dark": False,
        "title": "عام غلطیاں ۴ و ۵",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>خود تصدیق کا گمان</span></li>'
            '<li><span class="dot"></span><span>پالیسی نظرانداز کرنا</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-ur-07.png",
        "dark": False,
        "title": "پانچ سوال",
        "body": (
            '<div class="truths">'
            '<div class="truth"><span class="n">۱</span><span>حقیقت، عدد، نام جانچا؟</span></div>'
            '<div class="truth"><span class="n">۲</span><span>غلط ہونے پر ذمہ دار بننے کو تیار؟</span></div>'
            '<div class="truth"><span class="n">۳</span><span>مسودہ یا آؤٹ سورس؟</span></div>'
            '<div class="truth"><span class="n">۴</span><span>پالیسی کیا ہے؟</span></div>'
            '<div class="truth"><span class="n">۵</span><span>اپنی رائے شامل کی؟</span></div>'
            "</div>"
        ),
    },
    {
        "name": "POST-014-slide-ur-08.png",
        "dark": True,
        "title": "یاد رکھیں",
        "body": (
            '<p class="sub" style="font-size:32px">مسودہ اے آئی سے، فیصلہ اور ذمہ داری آپ کی۔</p>'
            '<p class="sub" style="margin-top:20px">اے آئی خواندگی سیریز جاری ہے</p>'
            '<div class="cta-pill">محفوظ کریں · فالو کریں · پوسٹ ۱۴</div>'
        ),
    },
]
...
SLIDES_EN = [
    {
        "name": "POST-014-slide-en-01.png",
        "dark": True,
        "title": 'Is AI the draft, or the <span class="hl">decision</span>?',
        "body": '<p class="sub">Verify, then send</p><p class="sub" style="margin-top:18px">Post 14</p>',
    },
    {
        "name": "POST-014-slide-en-02.png",
        "dark": False,
        "title": "Sent it unread",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Pasted the AI report</span></li>'
            '<li><span class="dot"></span><span>A wrong number was in it</span></li>'
            '<li><span class="dot"></span><span>Client caught it</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-en-03.png",
        "dark": False,
        "title": "From Post 13",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Confident wrong answers where patterns are thin</span></li>'
            '<li><span class="dot"></span><span>Today: the verification habit</span></li>'
            '<li><span class="dot"></span><span>Same habit continues</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-en-04.png",
        "dark": False,
        "title": "The actual split",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>AI gives the draft</span></li>'
            '<li><span class="dot"></span><span>You verify it</span></li>'
            '<li><span class="dot"></span><span>You decide, you\'re accountable</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-en-05.png",
        "dark": False,
        "title": "Common mistakes 1-3",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Copy-paste submission</span></li>'
            '<li><span class="dot"></span><span>Unverified facts</span></li>'
            '<li><span class="dot"></span><span>Outsourced big decisions</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-en-06.png",
        "dark": False,
        "title": "Common mistakes 4-5",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Self-check assumption</span></li>'
            '<li><span class="dot"></span><span>Skipped disclosure</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-014-slide-en-07.png",
        "dark": False,
        "title": "Five questions",
        "body": (
            '<div class="truths">'
            '<div class="truth"><span class="n">1</span><span>Fact, number, name checked?</span></div>'
            '<div class="truth"><span class="n">2</span><span>Ready to be accountable if wrong?</span></div>'
            '<div class="truth"><span class="n">3</span><span>Draft or outsourced?</span></div>'
            '<div class="truth"><span class="n">4</span><span>What\'s the policy?</span></div>'
            '<div class="truth"><span class="n">5</span><span>Added your own judgment?</span></div>'
            "</div>"
        ),
    },
    {
        "name": "POST-014-slide-en-08.png",
        "dark": True,
        "title": "Remember",
        "body": (
            '<p class="sub" style="font-size:30px">The draft can come from AI. The decision and responsibility stay with you.</p>'
            '<p class="sub" style="margin-top:20px">AI Literacy series continues</p>'
            '<div class="cta-pill">Save · Follow · Post 14</div>'
        ),
    },
]
...
def main() -> None:
    ...
    for lang in ("ur", "en"):
        ...
        html_path.write_text(html, encoding="utf-8")
        chrome_shot(html_path, ROOT / f"POST-014-hero-{lang}-1080x1080.png")
    ...
    print("POST-014 visual set complete ->", ROOT)
