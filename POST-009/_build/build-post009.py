...
BASE_CSS = f"""
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .split {{ display: flex; flex-direction: column; gap: 14px; margin-top: 12px; }}
  .lane-mini {{
  ...
  .lane-mini.local {{ background: rgba(10,85,92,0.08); border: 2px solid {COLORS["teal"]}; }}
  .lane-mini.local .tag {{ background: {COLORS["teal"]}; color: #fff; }}
  .lane-mini.other {{ background: rgba(232,184,74,0.14); border: 2px solid {COLORS["amber"]}; }}
  .lane-mini.other .tag {{ background: {COLORS["amber"]}; color: {COLORS["ink"]}; }}
"""
...
def hero_html(lang: str) -> str:
    ...
    badge = "شہری خواندگی · پوسٹ ۹" if is_ur else "Civic Literacy · Post 9"
    hook = (
        "مقامی حکومت کس کام کی ہے؟"
        if is_ur
        else 'What is local government actually <span class="hl">for</span>?'
    )
    sub = "صحیح دروازہ کیسے پہچانیں" if is_ur else "How to find the right door"
    panel = "غلط دروازہ · صحیح دروازہ" if is_ur else "Wrong door · right door"
    b_tag = "غلط؟" if is_ur else "Wrong?"
    b_h = "بڑا لیڈر ٹیگ" if is_ur else "Tag a big name"
    b_d = "لائٹ / کچرا / سڑک" if is_ur else "Light / trash / road"
    b_mega = "کوئی جواب نہیں" if is_ur else "No reply"
    b_status = "ناکافی" if is_ur else "Not enough"
    r_tag = "صحیح؟" if is_ur else "Right?"
    r_h = "صحیح دفتر" if is_ur else "Right office"
    r_d = "سطح · خدمت · دفتر" if is_ur else "Tier · service · office"
    checks = (
        [("۱", "سطح"), ("۲", "خدمت"), ("۳", "دفتر")]
        if is_ur
        else [("1", "Tier"), ("2", "Service"), ("3", "Office")]
    )
    r_status = "پڑھنے کی عادت" if is_ur else "The right habit"
    take = "مقامی حکومت روزمرہ خدمات کا ادارہ ہے، سیاسی میدان نہیں" if is_ur else "Local government is a service institution, not a political battlefield"
    footer = (
        "پاکستان میڈیا او ایس · شہری خواندگی · پوسٹ ۹"
        if is_ur
        else "Pakistan Media OS · Civic Literacy · Post 9"
    )
    check_rows = "".join(
...
def shell(lang: str, dark: bool, title: str, body_html: str, footer: str) -> str:
    ...
    series = "شہری خواندگی · پوسٹ ۹" if lang == "ur" else "Civic Literacy · Post 9"
...
FOOT_UR = "پاکستان میڈیا او ایس · شہری خواندگی · پوسٹ ۹"
FOOT_EN = "Pakistan Media OS · Civic Literacy · Post 9"

SLIDES_UR = [
    {
        "name": "POST-009-slide-ur-01.png",
        "dark": True,
        "title": "مقامی حکومت کس کام کی ہے؟",
        "body": '<p class="sub">سطح · خدمت · صحیح دفتر</p><p class="sub" style="margin-top:18px">پوسٹ ۹</p>',
    },
    {
        "name": "POST-009-slide-ur-02.png",
        "dark": False,
        "title": "لائٹ بند، کچرا نہیں اٹھا",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>بڑے لیڈر کو ٹیگ کیا</span></li>'
            '<li><span class="dot"></span><span>جواب نہ آیا</span></li>'
            '<li><span class="dot"></span><span>غلط دروازہ</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-ur-03.png",
        "dark": False,
        "title": "پوسٹ ۸ سے",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>جاری کنندہ چیک کرو</span></li>'
            '<li><span class="dot"></span><span>آج: وہ کس کام کا ہے</span></li>'
            '<li><span class="dot"></span><span>عادت جاری رہتی ہے</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-ur-04.png",
        "dark": False,
        "title": "مقامی بمقابلہ الگ",
        "body": (
            '<div class="split">'
            '<div class="lane-mini local"><span class="tag">مقامی</span><br>سڑک · صفائی · پانی · رجسٹریشن</div>'
            '<div class="lane-mini other"><span class="tag">الگ</span><br>بجلی · گیس · بڑی پالیسی</div>'
            "</div>"
        ),
    },
    {
        "name": "POST-009-slide-ur-05.png",
        "dark": False,
        "title": "عام الجھنیں ۱ تا ۳",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>سطح کی الجھن</span></li>'
            '<li><span class="dot"></span><span>سب کچھ سیاست</span></li>'
            '<li><span class="dot"></span><span>صرف نوکریوں کا ادارہ</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-ur-06.png",
        "dark": False,
        "title": "عام الجھنیں ۴ و ۵",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>غلط دروازہ</span></li>'
            '<li><span class="dot"></span><span>لاتعلقی</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-ur-07.png",
        "dark": False,
        "title": "پانچ سوال",
        "body": (
            '<div class="truths">'
            '<div class="truth"><span class="n">۱</span><span>کون سی سطح؟</span></div>'
            '<div class="truth"><span class="n">۲</span><span>کون سی خدمت؟</span></div>'
            '<div class="truth"><span class="n">۳</span><span>کون سا دفتر؟</span></div>'
            '<div class="truth"><span class="n">۴</span><span>شکایت کہاں؟</span></div>'
            '<div class="truth"><span class="n">۵</span><span>صحیح جگہ؟</span></div>'
            "</div>"
        ),
    },
    {
        "name": "POST-009-slide-ur-08.png",
        "dark": True,
        "title": "یاد رکھیں",
        "body": (
            '<p class="sub" style="font-size:32px">مقامی حکومت روزمرہ خدمات کا ادارہ ہے، سیاسی میدان نہیں۔</p>'
            '<p class="sub" style="margin-top:20px">شہری خواندگی سیریز فی الحال یہاں رکتی ہے</p>'
            '<div class="cta-pill">محفوظ کریں · فالو کریں · پوسٹ ۹</div>'
        ),
    },
]
...
SLIDES_EN = [
    {
        "name": "POST-009-slide-en-01.png",
        "dark": True,
        "title": 'What is local government actually <span class="hl">for</span>?',
        "body": '<p class="sub">Tier · service · right office</p><p class="sub" style="margin-top:18px">Post 9</p>',
    },
    {
        "name": "POST-009-slide-en-02.png",
        "dark": False,
        "title": "Light out, garbage uncollected",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Tagged a famous politician</span></li>'
            '<li><span class="dot"></span><span>No reply</span></li>'
            '<li><span class="dot"></span><span>Wrong door</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-en-03.png",
        "dark": False,
        "title": "From Post 8",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Check the issuer</span></li>'
            '<li><span class="dot"></span><span>Today: what it is actually for</span></li>'
            '<li><span class="dot"></span><span>Same habit continues</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-en-04.png",
        "dark": False,
        "title": "Local vs elsewhere",
        "body": (
            '<div class="split">'
            '<div class="lane-mini local"><span class="tag">LOCAL</span><br>Roads · sanitation · water · registration</div>'
            '<div class="lane-mini other"><span class="tag">ELSEWHERE</span><br>Electricity · gas · big policy</div>'
            "</div>"
        ),
    },
    {
        "name": "POST-009-slide-en-05.png",
        "dark": False,
        "title": "Mix-ups 1-3",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Tier confusion</span></li>'
            '<li><span class="dot"></span><span>All-politics lens</span></li>'
            '<li><span class="dot"></span><span>Jobs-office myth</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-en-06.png",
        "dark": False,
        "title": "Mix-ups 4-5",
        "body": (
            '<ul class="bullets">'
            '<li><span class="dot"></span><span>Wrong door</span></li>'
            '<li><span class="dot"></span><span>Apathy</span></li>'
            "</ul>"
        ),
    },
    {
        "name": "POST-009-slide-en-07.png",
        "dark": False,
        "title": "Five questions",
        "body": (
            '<div class="truths">'
            '<div class="truth"><span class="n">1</span><span>Which tier?</span></div>'
            '<div class="truth"><span class="n">2</span><span>Which service?</span></div>'
            '<div class="truth"><span class="n">3</span><span>Which office?</span></div>'
            '<div class="truth"><span class="n">4</span><span>Where to complain?</span></div>'
            '<div class="truth"><span class="n">5</span><span>Right place?</span></div>'
            "</div>"
        ),
    },
    {
        "name": "POST-009-slide-en-08.png",
        "dark": True,
        "title": "Remember",
        "body": (
            '<p class="sub" style="font-size:32px">Local government is a service institution, not a political battlefield.</p>'
            '<p class="sub" style="margin-top:20px">The Civic Literacy series pauses here for now</p>'
            '<div class="cta-pill">Save · Follow · Post 9</div>'
        ),
    },
]
...
def main() -> None:
    ...
    for lang in ("ur", "en"):
        ...
        html_path.write_text(html, encoding="utf-8")
        chrome_shot(html_path, ROOT / f"POST-009-hero-{lang}-1080x1080.png")
    ...
    print("POST-009 visual set complete ->", ROOT)
